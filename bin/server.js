...
const localPath = config.JSRF_PATH;
...
function getIPHash(user_ip) {
    if (typeof(user_ip) != 'string') {
        ...
        return crypto.createHash('sha256').update(user_ip).digest('hex');
    }
}
...
http.createServer(function(req, res) {
    ...
    if (req.method === "GET") {
        ...
        */
        ...
        // check if requests exists
        else if (fs.existsSync(localPath + decodeURI(pathname))) {
            // Check if request is directory
            if (!fs.lstatSync(localPath + decodeURI(pathname)).isDirectory()) {
            ...
            // if Directory return 404
                ...
                console.log("Directory return 404")
                res.statusCode = 404;
                res.setHeader('Location', config.HOST + '/404/404.html');
