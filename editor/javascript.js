function editorSizeAndPosition()
	...
	document.getElementById(e).style.width=Math.round(.25*parseInt(document.getElementById("editor").style.width))+"px",document.getElementById(e).style.height=Math.round(parseInt(document.getElementById(e).style.width)/t)+"px",document.getElementById(e).style.left=Math.round(0*parseInt(document.getElementById("editor").style.width))+"px",document.getElementById(e).style.top=1.1*-parseInt(document.getElementById(e).style.height)+"px";
	e="loadEditorButton",t=1;
	document.getElementById(e).style.width=Math.round(.25*parseInt(document.getElementById("editor").style.width))+"px",document.getElementById(e).style.height=Math.round(parseInt(document.getElementById(e).style.width)/t)+"px",document.getElementById(e).style.left=Math.round(.2*parseInt(document.getElementById("editor").style.width))+"px",document.getElementById(e).style.top=1.1*-parseInt(document.getElementById(e).style.height)+"px";
...
function searchForTrackNext()
	{
	for(searchTerm=document.getElementById("searchTextField").value,searchTermUppercase=searchTerm.toUpperCase(),trackSearch++,searchTermFlag=!1,t=trackSearch;
	t<=this[stationsArray[searchStation]+"_tracks"].length-1;
	t++)
		{
		var e=this[stationsArray[searchStation]+"_tracks"][t],n=e.toUpperCase();
...
function saveRequest()
	{
	document.getElementById("saveRequestButton").style.opacity=.3,scriptPassword=queryValue("p"),eventTypeSetting="REQUEST",eventVar1Setting=stationsArray[searchStation],eventVar2Setting=this[stationsArray[searchStation]+"_tracks"][trackSearch],eventVar1Setting=eventVar1Setting.replace("&","%26amp;"),
	eventVar2Setting=eventVar2Setting.replace("&","%26amp;"),
	postMessage="scriptPassword="+scriptPassword+"&eventType="+eventTypeSetting+"&eventVar1="+eventVar1Setting+"&eventVar2="+eventVar2Setting;
	var e=new XMLHttpRequest;
	e.open("POST","editor/saveEvent.php",!0),e.setRequestHeader("Content-type","application/x-www-form-urlencoded"),e.send(postMessage),e.onreadystatechange=function()
...
function saveTicker()
	{
	document.getElementById("saveTickerButton").style.opacity=.3,scriptPassword=queryValue("p"),message=document.getElementById("tickerEditorTextField").value,message=message.replace("&","%26amp;"),
	postMessage="scriptPassword="+scriptPassword+"&message="+message;
	var e=new XMLHttpRequest;
...
function editorRuntime()
	{
	allSizeAndPositionFunctionsList[allSizeAndPositionFunctionsList.length]=editorSizeAndPosition,allSizeAndPositionFunctionsList[allSizeAndPositionFunctionsList.length-1](),editorSwitch(),defineEditorButtons(),defaultEditor(),defineEditorTextFieldFocus(),loadTickerData()
}
